...
class TestRepresentationSubspace(unittest.TestCase):
    ...
    #     test_groups = [(SO(2),S(5))]#,(S(5),SO(3))]
    #     for G1,G2 in test_groups:
    #         # repa = (T(0)+T(1)+T(2))(G1)
    #         # print(f"did repa {repa.shape} {repa.size()}")
    #         # repb = (T(0)+T(1))(G2)
    #         # print(f"did repb {repb.shape} {repb.size()}")
    #         # repin = repa*repb
    #         repin = T(1)(G2)+T(1)(G1)*T(1)(G2)
    #         #print(f"did repin {repin.shape} {repin.size()}")
    #         repout = (T(1)(G1) + T(2)(G1)*T(0)(G2) + T(1)(G1)*T(1)(G2) + T(0)(G1)+T(2)(G1)*T(1)(G2))
    #         #print(f"did repout {repout.shape} {repout.size()}")
    #         repW = repout*repin.T
    #         #print(f"did repW {repW.shape} {repW.size()}")
    ...
    #         samples = {G1:G1.samples(N),G2:G2.samples(N)}
    #         ring = vmap(repin.rho)(samples)
    #         routg = vmap(repout.rho)(samples)
    #         gx = (ring@x[...,None])[...,0]
    #         Wgx =gx@W.T
    #         #print(g.shape,(x@W.T).shape)
    #         gWx = (routg@(x@W.T)[...,None])[...,0]
