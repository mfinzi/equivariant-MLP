...
class TestRepresentationSubspace(unittest.TestCase):
    test_groups = [SO(n) for n in [1,2,3,4]]+[O(n) for n in [1,2,3,4]]+\
                    ...
                    [DiscreteTranslation(n) for n in [2,5,6]]
                    #+[SO13p(),SO13(),O13()] ##[Symplectic(n) for n in [1,2,3,4]]+
        ...
        for G in self.test_groups:
            for p in range(r+1):
                for q in range(r-p+1):
                    ...
                    self.assertTrue(err<1e-3,f"Symmetric vector fails err {err:.3e} with T{p,q} and G={G}")
